...
const METHODS  = ['Tarjeta de crédito', 'Tarjeta de débito', 'Transferencia bancaria', 'Pago en efectivo'];
  ...
  if (method === 'Tarjeta de crédito' || method === 'Tarjeta de débito') {
    return (
      <div className="space-y-3 border rounded-xl p-4 bg-gray-50">
        ...
        {field('cardNumber', 'Número de tarjeta',      '1234 5678 9012 3456')}
  ...
  if (method === 'Transferencia bancaria') {
    return (
      <div className="space-y-3 border rounded-xl p-4 bg-gray-50">
        ...
        {field('transferRef',  'Número de referencia', 'REF-123456')}
  ...
  if (method === 'Pago en efectivo') {
    return (
      <div className="space-y-3 border rounded-xl p-4 bg-gray-50">
        ...
        <div className="bg-white rounded-lg p-3 text-sm text-gray-600 space-y-1 border">
          <p>Podrás pagar en efectivo al momento de recibir tu pedido.</p>
          <p className="mt-2"><span className="font-medium">Horario de entrega:</span> Lunes a Sábado 8am - 6pm</p>
...
export default function Checkout() {
  ...
  const [form, setForm] = useState({
    name: '', address: '', city: 'Tegucigalpa', phone: '', email: '', method: ''
  });
  ...
  // El envío gratis SOLO aplica a partir de L. 2,000.
  // Por debajo de ese monto, el costo lo determina calculateShipping() según ciudad.
  ...
  const applyCoupon = () => {
    ...
    if (!found) { setDiscount(0); setCouponMsg('Código inválido'); setAppliedCoupon(null); return; }

    if (new Date() > new Date(found.expires)) {
      setDiscount(0); setCouponMsg('Cupón expirado'); setAppliedCoupon(null); return;
    }
    if (baseTotal < found.min) {
      setDiscount(0); setCouponMsg(`Mínimo L. ${found.min}`); setAppliedCoupon(null); return;
    }

    const d = found.type === 'percent' ? baseTotal * (found.value / 100) : found.value;
    setDiscount(d);
    setCouponMsg(`Cupón aplicado  −L. ${d.toFixed(2)}`);
  ...
  const handleSubmit = async () => {
    ...
    if (fields.some(f => newErrors[f])) return;   // ← bloquea si hay errores
    ...
    const order = {
      id: orderId, items: cart, subtotal, tax,
      shipping: shipping.cost, discount, total: finalTotal,
      customer: form, paymentData,
    ...
    <main className="bg-gray-50 min-h-screen py-10">
      <div className="max-w-5xl mx-auto px-6 grid md:grid-cols-2 gap-10">
        ...
        <div className="bg-white p-6 rounded-2xl space-y-4">
          ...
          {/* Ciudad — sin validación, valor siempre válido */}
          <div>
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-gray-700">Método de pago</h3>
            {METHODS.map(m => (
          ...
          {form.method && (
            <PaymentForm method={form.method} paymentData={paymentData} setPaymentData={setPaymentData} />
          )}
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl h-fit sticky top-24">
          ...
          <div className="border-t pt-3 space-y-2 text-sm">
            ...
            {/* ── Fila de envío: muestra costo por ciudad o "Gratis" ── */}
            <div className="flex justify-between">
              <span>Envío{form.city ? ` — ${form.city}` : ''}</span>
              <span className={shippingFree ? 'text-green-600 font-medium' : ''}>
                {shippingFree ? 'Gratis' : `L. ${shipping.cost.toFixed(2)}`}
              </span>
            </div>

            {/* Mensaje motivacional: solo si aún no alcanzó el umbral */}
            {!shippingFree && (
              <p className="text-xs text-gray-400">
                Agrega L. {(FREE_SHIPPING_THRESHOLD - subtotal).toFixed(2)} más para obtener envío gratis.
              </p>
            )}
          ...
          <div className="mt-4 text-xs text-gray-400 space-y-1">
            ...
            <p className="flex items-center gap-1"><Truck size={13} /> Envío rastreable</p>
          </div>
        </div>
